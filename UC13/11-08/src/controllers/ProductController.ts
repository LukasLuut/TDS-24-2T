import { Request, Response } from "express"
import { Product, produtos } from "../models/Product";

export class ProductController{
/*--------------------------------------------------------------------------------------------------------*/
    createProduct(req: Request, res: Response): Response {
        const { id, nome, qdt, preco } = req.body;

        if (!id || !nome || !qdt || !preco) {
            return res.status(400).json({ mensagem: "Id, nome, qdt e preco precisam ser informados!" });
        }

        const produto = new Product(id, nome, qdt, preco);
        produtos.push(produto);

        return res.status(201).json({ mensagem: "Produto criado com sucesso!", produto: produto });

    }
/*--------------------------------------------------------------------------------------------------------*/
    listAllProducts(req: Request, res: Response): Response {
        return res.status(200).json({ products: produtos });
    }
/*--------------------------------------------------------------------------------------------------------*/
    updateProduct(req: Request, res: Response): Response {
        const id: number = Number(req.params.id); //esse params fala sobre a rota que vai no Thunder, aqui ele está convertendo de string para Number
        const { nome, qdt, preco } = req.body;

        if (!nome || !qdt || !preco) {
            return res.status(400).json({ mensagem: "Nome e e-mail são obrigatórios!" })
        }

        let produto = produtos.find(produto => Number(produto.id) === id);//.find é uma função para array, para encontrar o id

        if (!produto) return res.status(404).json({ mensagem: "Produto não encontrado!" })

        produto.nome = nome;
        produto.qdt = qdt;
        produto.preco=preco;

        return res.status(200).json({ mensagem: "Produto atualizado com sucesso!", produto_atualizado: produto })
}
/*--------------------------------------------------------------------------------------------------------*/
    deleteProduct(req: Request, res: Response): Response {
        const id: number = Number(req.params.id);

        let index = produtos.findIndex(prod => prod.id === id);

        if (index === -1) {
            return res.status(404).json({ mensagem: "Produto não encontrado" })
        }

        produtos.splice(index, 1);
        return res.status(204).send();
}


}